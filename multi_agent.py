...
_TEAM_KEYWORDS = {
    "新功能", "重构", "架构设计", "技术方案", "创建项目",
    "新增功能", "添加功能", "拆分成多个", "从零开始", "搭建",
}
_SINGLE_KEYWORDS = {
    ...
    "解决了吗", "完成了吗", "怎么样了",
    "改一下", "修一下", "优化一下", "帮忙看", "帮忙修",
    "写一下", "加一下", "部署", "发布", "上线",
    "修bug", "修复", "fix", "改个", "加个",
}

ROUTER_SYSTEM_PROMPT = """你是一个任务路由器。判断用户消息应该走 team 还是 single 模式。

team 模式（多步骤编排，需要计划+确认）：
- 明确的多文件/多步骤开发任务（"搭建一个新项目"、"重构认证模块"、"把 ws_bot 拆成多个文件"）
- 涉及架构设计、系统集成、从零开始构建

single 模式（直接执行，大多数任务都走这个）：
- 改 bug、修配置、查状态、部署、提交代码
- 单文件修改、简单功能添加
- 问答、解释、查询
- "帮我改一下"、"优化一下"、"加个功能"这类简单请求

原则：除非任务明确需要多步规划和用户确认，否则走 single。宁可 single 也不要误判为 team。

只回复 JSON：{"mode": "team"} 或 {"mode": "single"}"""
