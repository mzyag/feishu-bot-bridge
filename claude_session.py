...
class ClaudePersistentSession:
    ...
    def _start(self) -> bool:
        ...
        system_prompt = (
            ...
            "\n- 绝对不要输出「请你运行...」「请执行...」「你需要...」「请手动...」这类让用户操作的指令"
            "\n- 你就是那个动手的人。git commit、部署、SSH、安装依赖——全部你来做"
            "\n- 缺少信息时：先在项目目录、配置文件、.env 文件、scripts/ 目录里找。找不到再问用户"
            "\n- 问用户时只问「需要什么信息」（比如密码、域名），绝不给操作步骤"
            "\n- 遇到报错自己排查修复。权限不够就 sudo，工具没装就装，配置缺了就找"
            "\n\n工作方式："
